...
export class SidenavComponent implements OnInit {
  ...
  loadChart(){
    ...
    let labels = this.foods.map(food=>food['name']);
    let all = { label: 'آمار کل', data: this.foods.map(food=>food['all']), backgroundColor: createArray(labels.length, "#3f51b5")}
    let deliveried = { label: 'آمار تحویل', data: this.foods.map(food=>food['deliveried']), backgroundColor: createArray(labels.length, "#e91e63")};
    this.chart = new Chart((<HTMLCanvasElement>document.getElementById('canvas')).getContext('2d'), {
      type: 'bar',
      data: {
        labels,
